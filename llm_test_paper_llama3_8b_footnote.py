...
def build_content_trans_prompt(source_lang, trans_lang, data):    
    ...
    prompts = [f"Acts as a smart translator. Translate {source_lang} sentences into {trans_lang} sentences in written style. Do not remove heading word. Do not add any characters. If sentence includes '----footnotes----' then translate it. I need only translation sentence. '{string.strip()}'" for string in data if string.strip()]

    return prompts

def build_footnote_trans_prompt(source_lang, trans_lang, data):    
    # prompts = [f"Translate from {source_lang} into {trans_lang}. I need only translation sentence. Remove extra symbols and characters. '{sentence}'" for sentence in data if sentence.strip()]
    # prompts = [f"'{sentence}'" for sentence in data if sentence.strip()]
...
def main():  
    ...
    # system_prompt = f"Acts as a smart translator. Translate {source_lang} sentences into {trans_lang} sentences in {style} style. Do not remove heading word. Do not add any characters. If sentence includes '----footnotes----' then translate it. I need only translation sentence."

    # system_prompt_content = "Translate into English. I need only translation. Do not add others. Do not remove any word. Do not drop original sentence."
    # system_prompt_footnote = f"Acts as a smart translator. Translate {source_lang} sentences into {trans_lang} sentences in {style} style. Do not remove heading word. Do not add any characters. I need only translation sentence."
    ...
    if content_data:
        content_prompts = build_content_trans_prompt(source_lang=source_lang, trans_lang=trans_lang, data=content_data)
    else:
        content_prompts = ""   
    ...
    if content_prompts:
        for prompt in content_prompts:  
            if '----media/' in prompt:
                ...
                # prompt = system_prompt_content + prompt          
                prompt = f"Q: {prompt} A: "
    ...
    content_translated_file_path = 'content_test_result.docx'
    ...
    if footnote_para_indexes != []:
        ...
        if footnote_data:
            footnote_prompts = build_footnote_trans_prompt(source_lang=source_lang, trans_lang=trans_lang, data=footnote_data)
        else:
            footnote_prompts = ""
        ...
        for i, index in enumerate(footnote_para_indexes):
            ...
            remove_footnote_string_doc = Document(content_translated_file_path)
    ...
    if image_flag == True:
        result_file_path = "image_added_test_result.docx"
